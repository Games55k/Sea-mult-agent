watchdog（看门狗）是一个非常经典且带有极强系统底层色彩的命名！在嵌入式和高可用系统中，Watchdog Timer 本就是用来在系统死机或陷入死循环时执行硬件复位的终极手段。用在这里做 Agent 的底线防线，不仅贴切，而且非常有极客感。

这是为你重新编写的 watchdog 模块的 README.md：
